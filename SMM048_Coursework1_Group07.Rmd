...
Noting that the majority of claims amount is zero in the dataset (63232 out of 67856 records), we initially considered two broad approaches for the analyis.
...
The fitted model implies represent claim arrivals under the assumption that claims occur independently at a constant rate over the exposure period with an estimated annual claim rate of 〖μ ̂=e〗^(-2.66711)=0.0694 claims per unit exposure, indicating that claims are relatively infrequent events in the portfolio.
...
The fitted Inverse Gaussian model implies an average positive claim severity of approximately μ=exp⁡(7.60808)=2011, with variability increasing with the cube of the mean, which indicates the heavy right tail commonly observed in claim severity data.

## Model Diagnostics
Null hypothesis is data follow the fitted marginal distribution.

### Q-Q plots
The Q–Q plots obtained from res.check() show that the truncated Poisson model provides a satisfactory representation of claim frequency, while the inverse Gaussian distribution captures the overall shape of the severity distribution. Although the lower tail appears to deviate from the diagonal line, indicating that the model underfits very small claim amounts, the overall fit is considered adequate for modelling dependence in the following copula analysis. 

### Convergence & Information Matrix
...
Among these candidates, the Clayton copula provides the best fit, achieving the lowest AIC and BIC values by a clear margin. Convergence diagnostics confirm stable estimation, with a maximum gradient of 8.45 × 10⁻⁶ and a positive definite information matrix.
The estimated Clayton copula parameter is positive and statistically significant θ=1.47  , indicating positive dependence between claim frequency and claim severity. It corresponds to a Kendall’s τ =θ/(θ+2) =0.4236, which indicates a moderate positive association between claim frequency and claim severity.
...
## Fitting claim amount marginal distribution

Claim severity was modelled using positive claim amounts only, where the data exhibits a strongly right-skewed distribution. Among the models tested, the Inverse Gaussian distribution provides the best fit, achieving the lowest AIC and BIC values. Model diagnostics indicate satisfactory convergence near zero and a positive definite information matrix.

The Normal Q-Q plot shows a reasonable fit for the middle and upper quantiles, while the lower tail deviates from the diagonal line, indicating that the model underestimates very small claim amounts, but the overall fit is considered adequate for modelling dependence in the following copula analysis.
...
```{r ts2-aicbic, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Top 5 ARIMA models for time series 2 ranked by AIC and BIC", fig.show='hold', fig.align='center', fig.width=6.2, fig.height=3, fig.pos='H', out.width='45%'}
...
```{r ts3-aicbic, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Top 5 ARIMA models for time series 3 ranked by AIC and BIC", fig.show='hold', fig.align='center', fig.width=6.2, fig.height=3, fig.pos='H', out.width='45%'}
...
Goodness-of-fit tests were conducted using the Kolmogorov–Smirnov (KS) and Cramér–von Mises (CvM) statistics. The KS statistic (D1=0.93707, D2=0.74048) measures the maximum difference between the empirical and theoretical cumulative distributions of zero truncated poisson and inverse gaussian, while the Cramér–von Mises statistic (ω1²=1268.4, ω2²=653.09,) measures the overall squared discrepancy across two distributions. Both tests reject the null hypothesis that the observed data follow the fitted distributions exactly (p-value < 2.2 × 10⁻¹⁶). However, with a sample size of n = 4624, these tests are highly sensitive and may detect even minor deviations from the theoretical distribution. 
