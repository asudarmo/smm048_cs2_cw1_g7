...
## Fitting claim frequency marginal distribution

For claim frequency, the models were fitted using exposure as an offset to account for varying policy durations. Although the truncated Poisson–Inverse Gaussian model yields the lowest AIC, the improvement over the truncated Poisson model is negligible (𝛥𝐴𝐼𝐶 ≈ 1.13). In contrast,the truncated Poisson model achieves the lowest BIC due to its model simplicity. Therefore, the truncated Poisson model was selected as the marginal distribution for claim frequency. The fitted model implies an estimated annual claim rate of $\mu \approx e^{−2.66711} = 0.0694$ claims per unit exposure. Model diagnostics indicate satisfactory convergence with the maximum absolute gradient close to zero, a positive definite information matrix, and a closely fitted Normal QQ-plot.
